...
pub struct ConnectedTcpSocket<'i, 's, 'sockset> {
    iface:   &'i NetworkInterfaceRef, 
    sockets: &'s mut SocketSet<'sockset>,
    handle:  SocketHandle,
}
impl<'i, 's, 'sockset> ConnectedTcpSocket<'i, 's, 'sockset> {
    ...
    ) -> Result<ConnectedTcpSocket<'i, 's, 'sockset>, &'static str> {
